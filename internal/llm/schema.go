...
func BuildReceiptJSONSchema(allowedCategories []string) map[string]any {
	props := map[string]any{
		...
		"subtotal":      map[string]any{"type": "string", "pattern": `^\d+(\.\d{2})?$`},
		"tax":           map[string]any{"type": "string", "pattern": `^\d+(\.\d{2})?$`},
		"total":         map[string]any{"type": "string", "pattern": `^\d+(\.\d{2})?$`},
		"currency_code": map[string]any{"type": "string", "pattern": `^[A-Z]{3}$`},
		"category":      map[string]any{"type": "string"},
		"payment_method": map[string]any{
			"type": "string",
		},
		"payment_last4": map[string]any{
			"type":      "string",
			"pattern":   `^\d{4}$`,
			"minLength": 4,
			"maxLength": 4,
